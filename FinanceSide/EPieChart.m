...
//@synthesize pourcentageLoose = _pourcentageLoose;
 ...
 ePieChartDataModel:(EPieChartDataModel *)ePieChartDataModel
{
    ...
    if (self)
    {
        ...
        if (nil == ePieChartDataModel)
        {
            ...
            _percentage = [[EPie alloc] initWithCenter: CGPointMake(CGRectGetWidth(self.bounds) / 2.0 + MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 2.5    +  (MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 2.5) / 4, CGRectGetHeight(self.bounds) / 2.0)
                                              radius: MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 2.5 ];
        }
        else
        {
            ...
            _percentage = [[EPie alloc] initWithCenter: CGPointMake(CGRectGetWidth(self.bounds) / 2.0 + MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 2.5   +  (MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 2.5) / 4   , CGRectGetHeight(self.bounds) / 2.0)
                                                radius: MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)) / 8 ];
        }
        ...
        _percentage.layer.shadowOffset = CGSizeMake(0, 3);
        _percentage.layer.shadowRadius = 5;
        _percentage.layer.shadowColor = EGrey.CGColor;
        _percentage.layer.shadowOpacity = 0.8;
        _percentage.layer.backgroundColor = [[UIColor colorWithRed:255.0/255.0 green:1 blue:1 alpha:1] CGColor];
        [self addSubview: _percentage];
        
        
        
        _pourcentageLoose =  (_ePieChartDataModel.current / _ePieChartDataModel.budget )*100;
        
        UICountingLabel *percentLabel = [[UICountingLabel alloc] initWithFrame:self.frame];
        percentLabel.center = CGPointMake(CGRectGetMidX(_percentage.bounds), CGRectGetMidY(_percentage.bounds) * 1);
        percentLabel.textAlignment = NSTextAlignmentCenter;
        percentLabel.method = UILabelCountingMethodEaseInOut;
        percentLabel.font = [UIFont fontWithName:@"Menlo" size:10];
        percentLabel.textColor = [UIColor blackColor];
        percentLabel.format = @"%.1f%%";
        [_percentage addSubview:percentLabel];
        [percentLabel countFrom:0 to: _pourcentageLoose withDuration:2.0f];
    ...
    [UIView transitionWithView:self
                    ...
                    animations:^
     {
         ...
         else
         {
             ...
             [_percentage removeFromSuperview];
...
//        [_contentView addSubview:line];
        ...
        budgetLabel.font = [UIFont fontWithName:@"Menlo" size:25];
        budgetLabel.textColor = [UIColor whiteColor];
        budgetLabel.format = @"%.1f %%";
        [_contentView addSubview:budgetLabel];
        [budgetLabel countFrom:0 to: (_ePieChartDataModel.estimate / _ePieChartDataModel.budget )*100 withDuration:2.0f];
...
- (void) reloadContent
{
    ...
    UIBezierPath* circleEstimatePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds))
                                                                      radius: _radius * 0.8
                                                                  startAngle: 0
                                                                    endAngle: M_PI  - (_ePieChartDataModel.estimate / _ePieChartDataModel.budget) * (M_PI )
                                                                   clockwise:NO];
    if (!_circleEstimate)
    ...
    _circleEstimate.lineCap = kCALineCapSquare;
    ...
    _circleCurrent.lineCap = kCALineCapSquare;
